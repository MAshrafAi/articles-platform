...
- 2026-04-24 — **Dropped the `public.prompts` table** and reverted Phase 3's admin-editable prompts. The table was never actually wired to the edge functions (they always imported from `supabase/functions/prompts/*.ts`), so dropping it has no runtime effect. Prompt edits now happen by editing the `.ts` files under [supabase/functions/prompts/](supabase/functions/prompts/) and redeploying the affected function. Removed: migration `20260423000001_prompts_table.sql`, empty stub `app/(protected)/settings/prompts/`. Remote migration history repaired (`supabase migration repair --status reverted`) so local and remote match. (prompts / schema)
